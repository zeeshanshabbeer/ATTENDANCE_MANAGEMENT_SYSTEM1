...
exports.attendanceSheet=catchAsync(async(req,res,next)=>{
    const {user_id}=req.user
    const users= await Attendance.findOne({
        attendance_student_id:user_id
    })
    ...
    worksheet.columns=[
      ...
      {header:"Attendance Type",key:"attendance_type",width:20},
      {header:"Date",key:"date",width:20},
      {header:"Description",key:"attendance_description",width:40},
    ]
    let count=1
    users.request.forEach(user=>{
      user.s_no=count;
    ...
    const data=await workbook.xlsx.writeFile('Attendance_Sheet.xlsx')
      res.send("done")
  })